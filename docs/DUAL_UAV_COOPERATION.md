...
从已验证单机迁移到双机的网络、编译、坐标标定、启动顺序和验收步骤，见
[`DUAL_UAV_DEPLOYMENT_GUIDE.md`](DUAL_UAV_DEPLOYMENT_GUIDE.md)。
...
4. Explorer 同时接收本机和同伴 PVBSM。跨机PVBSM先经过带会话、序号、校验、缺口请求和有界重放缓存的同步中继，再按序进入接收端。查询远程覆盖前，通过 TF 将本机候选变换到远程 PVBSM 坐标；多来源结果按“任一来源已覆盖”保守融合，用于抑制重复探索。
...
- 本版使用同一 ROS master（或可透明转发 ROS 话题的局域网）通信；已经实现ROS消息层校验、序号缺口检测、NACK和有界缓存补发，但尚未实现独立UDP分片、无线带宽整形和超出缓存窗口后的完整快照回退。
- 跨机坐标由静态 TF/起飞标定给出；尚未实现基于公共子地图的自动在线对齐和错误对齐撤销。
- 当前租约粒度是“已选目标及空间排斥半径”，不是完整 Frontier 簇拍卖；已能避免两机同时前往同一区域，但负载均衡仍可扩展。
- 同伴失联后租约过期，本机将其目标登记为有限时长的接管区域，并重新生成满足本机安全约束的Frontier观察点；持久化区域任务账本仍需后续实现。
